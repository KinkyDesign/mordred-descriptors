...
async def apply_descriptor(request: Request):
    ...
    for entry in request.dataset.dataEntry:
        smiles.append(entry.values[key])
    ...
    descriptor_names_all = Calculator(descriptors, ignore_3D=True).name_dict
    descriptor_names_list = []
    for key, value in descriptor_names_all.items():
    ...
    features_out = [FeaturesOut(name=d + " descriptor of feature with name smiles and URI " + request.dataset.features[0].uri, conditions=conditions_mordred, category="CDK", uri=d) for d in descriptor_names_list]
...
if __name__ == "__main__":
    uvicorn.run("mordred-api:app", host="0.0.0.0", port=8000, reload=True)
